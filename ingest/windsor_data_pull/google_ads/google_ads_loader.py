"""Windsor.ai Google Ads connector loader.

Raw target : raw_windsor.google_ads  (the shared raw layer; project agora-data-driven,
             dataset raw_windsor, location asia-southeast1).
Source     : Windsor.ai -- google_ads connector (Google Ads).
Cadence    : daily scheduled pull (see scripts/deploy_ingest_jobs.ps1, cron 15 1 * * *).
...
rebuilding. This loader just keeps raw_windsor.google_ads fresh once a day.
...
def fetch_rows(api_key: str) -> list:
    """Pull Google Ads metrics from the Windsor REST API and return rows for BigQuery.

    Returns a list of dicts whose keys match the raw_windsor.google_ads schema
    (date, campaign, spend, conversions, revenue).
    """
    # TODO: Build the real Windsor request for the Google Ads connector. Windsor's
    # "all" endpoint takes an api_key plus a connector/datasource id, a field list, and
    # a date range, e.g.:
    #     params = {
    #         "api_key": api_key,
    #         "date_preset": "last_30d",
    #         "fields": "date,campaign,spend,conversions,revenue",
    #         "connector": "google_ads",   # confirm the real connector id
    #     }
    #     resp = requests.get(WINDSOR_API_BASE, params=params, timeout=120)
    #     resp.raise_for_status()
    #     payload = resp.json()["data"]
    # TODO: Map Windsor's returned field names to the raw_windsor.google_ads columns.
    # The exact Windsor field labels depend on the account's connector config.
    raise NotImplementedError(
        "TODO: implement the Windsor Google Ads request + field mapping for this account"
    )


def load_rows(bq: bigquery.Client, rows: list) -> None:
    """Truncate-and-load ``rows`` into raw_windsor.google_ads (idempotent refresh)."""
