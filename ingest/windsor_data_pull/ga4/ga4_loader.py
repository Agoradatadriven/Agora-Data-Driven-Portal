"""Windsor.ai GA4 connector loader.

Raw target : raw_windsor.ga4  (the shared raw layer; project agora-data-driven,
             dataset raw_windsor, location asia-southeast1).
Source     : Windsor.ai -- ga4 connector (Google Analytics 4).
Cadence    : daily scheduled pull (see scripts/deploy_ingest_jobs.ps1, cron 10 1 * * *).
...
rebuilding. This loader just keeps raw_windsor.ga4 fresh once a day.
...
def fetch_rows(api_key: str) -> list:
    """Pull GA4 metrics from the Windsor REST API and return rows for BigQuery.

    Returns a list of dicts whose keys match the raw_windsor.ga4 schema
    (date, channel, sessions, users, conversions).
    """
    # TODO: Build the real Windsor request for the GA4 connector. Windsor's "all"
    # endpoint takes an api_key plus a connector/datasource id, a field list, and a
    # date range, e.g.:
    #     params = {
    #         "api_key": api_key,
    #         "date_preset": "last_30d",
    #         "fields": "date,channel,sessions,users,conversions",
    #         "connector": "google_analytics_4",   # confirm the real connector id
    #     }
    #     resp = requests.get(WINDSOR_API_BASE, params=params, timeout=120)
    #     resp.raise_for_status()
    #     payload = resp.json()["data"]
    # TODO: Map Windsor's returned field names to the raw_windsor.ga4 columns. The
    # exact Windsor field labels depend on the account's connector config.
    raise NotImplementedError(
        "TODO: implement the Windsor GA4 request + field mapping for this account"
    )


def load_rows(bq: bigquery.Client, rows: list) -> None:
    """Truncate-and-load ``rows`` into raw_windsor.ga4 (idempotent full refresh)."""
    ...
    # TODO: switch to an incremental MERGE keyed on (date, channel) if/when the
