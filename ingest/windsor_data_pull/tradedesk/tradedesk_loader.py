"""Windsor.ai -> raw_windsor.tradedesk ingest loader.

Raw target : raw_windsor.tradedesk (the shared Windsor mirror dataset)
Source     : Windsor.ai "The Trade Desk" connector (programmatic DSP spend/delivery)
...
TABLE = "tradedesk"  # raw_windsor.tradedesk
WINDSOR_SECRET = "windsor-api-key"

# Windsor.ai REST endpoint. The connector slug identifies The Trade Desk.
WINDSOR_BASE_URL = "https://connectors.windsor.ai/all"
# TODO: confirm the exact Windsor connector slug for The Trade Desk (e.g.
#       "tradedesk" / "thetradedesk") against the Windsor.ai connector catalogue.
WINDSOR_CONNECTOR = "tradedesk"
...
def fetch_windsor_rows(api_key):
    """Pull The Trade Desk rows from the Windsor.ai REST API.

    Returns a list of dicts, one per source row, already mapped onto the
    raw_windsor.tradedesk schema (see create_tradedesk_table.py).
    """
    # TODO: build the real Windsor request for The Trade Desk. Windsor's /all
    #       endpoint takes the api_key, a connector selector and a comma-separated
    #       `fields` list, and (typically) a date range. Page through results if
    #       the connector returns more than one page.
    params = {
        "api_key": api_key,
        "connector": WINDSOR_CONNECTOR,
        # TODO: enumerate the real Windsor field names for The Trade Desk and map
        #       them to our column names below.
        "fields": "date,campaign,advertiser,impressions,clicks,spend,conversions",
    ...
    rows = []
    for r in raw_rows:
        # TODO: align these keys to Windsor's actual field names for The Trade Desk.
        rows.append(
            {
...
def load_rows(rows):
    """Replace raw_windsor.tradedesk with the freshly pulled rows.
...
def main():
    ...
    load_rows(rows)
    print("[OK] tradedesk ingest complete")
...
if __name__ == "__main__":
    try:
        ...
        print(f"[ERROR] tradedesk ingest failed: {exc}", file=sys.stderr)
        sys.exit(1)
