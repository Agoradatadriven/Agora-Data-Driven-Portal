# =============================================================================
# scheduler.ps1 -- create / refresh the */15 self-gating Cloud Scheduler trigger
#                  for the status dashboard export job.
#
# Creates (or updates, idempotently) the Cloud Scheduler HTTP job
# `status-export-daily`, which POSTs the Run Admin API :run URI of the
# `status-export` Cloud Run job every 15 minutes. The status export job is
# SELF-GATING: it runs on this */15 tick but only rebuilds when the shared
# raw_windsor mirror tables it gates on advanced past its _freshness.json
# watermark in the status bucket. So a frequent tick is cheap -- most ticks no-op.
#
...
$JOB   = "status-export"         # Cloud Run export job
$SCHED = "status-export-daily"   # Cloud Scheduler trigger
$CRON  = "*/15 * * * *"          # status tick; the job self-gates on the watermark
...
# (deploy_status.ps1 grants that). We resolve it from the project number rather
...
# =============================================================================
# Step 2 -- Create-or-update the */15 Cloud Scheduler HTTP job.
...
Write-Host "[OK] scheduled $SCHED -> $JOB (every 15 min; job self-gates on _freshness.json)" -ForegroundColor Green
