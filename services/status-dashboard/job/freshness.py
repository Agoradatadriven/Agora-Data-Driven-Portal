...
Why the consumers self-gate (and not the Windsor loaders):
  The Windsor ingest jobs are scheduled API pulls -- they are the WRITERS of `raw_windsor`, so there
  is nothing upstream of them to gate against. The self-gating lives in the CONSUMERS: each client
  export job (and the status dashboard) runs every few minutes and rebuilds only when the Windsor
