"""Flask web service for the status dashboard -- a META dashboard over ALL clients.

What it is:
  This is the freshness monitor for the whole fleet. Unlike a client dashboard, it has NO
  dataset and NO SQL views of its own -- it only renders the precomputed status.json that the
  status JOB writes. The status JOB walks every client's _freshness.json watermark + exported
  <c>.json and rolls them up into one status.json object in the status bucket.

Serving model -- private bucket + Flask password gate (NO SSO):
  status.json lives in a PRIVATE GCS bucket (agora-data-driven-status-dash) and is NEVER
  public. This service renders a login, holds a signed session, and proxies the private object
  at /data.json ONLY to an authenticated session. This is the SAME private-bucket serving
  pattern as a client dash, but password-gated ONLY -- there is no additive portal SSO here
  (the status dashboard is an internal operator tool, not a client-facing one), so it does NOT
  import platform_sso.
...
# (status-dash-session-key) at deploy time. A missing secret is a hard misconfig.
SESSION_SECRET = os.environ.get("SESSION_SECRET", "")
# The dashboard's own password (mounted from status-dash-password). compare_digest below is
# constant-time, so never compare it with `==`.
DASH_PASSWORD = os.environ.get("DASH_PASSWORD", "")
# The PRIVATE bucket + object holding the rolled-up fleet status JSON.
GCS_BUCKET = os.environ.get("GCS_BUCKET", "agora-data-driven-status-dash")
DATA_OBJECT = os.environ.get("DATA_OBJECT", "status.json")

app = Flask(__name__)
app.secret_key = SESSION_SECRET

# Cookie hardening. The status dashboard has no cross-subdomain SSO flow, but we keep the
# same hardened cookie config as the client dashes for consistency: Secure + HttpOnly. We
# leave SameSite at the Flask default here since there is no cross-site portal navigation to
# preserve a cookie across.
app.config.update(
...
def authed():
    """True iff this request is authenticated by its own session.

    Password-gated ONLY -- no additive portal SSO. Wrapped defensively so a missing/odd
    session never raises into the gate: any exception fails CLOSED (treated as unauthed).
    """
    try:
...
<title>Agora Data Driven -- Status sign in</title>
<style>
  :root {
...
<body>
  <form class="card" method="POST" action="/login">
    ...
    <div class="sub">Status dashboard -- sign in to view fleet freshness.</div>
...
def data_json():
    # The auth-gated proxy of the PRIVATE status object: only an authenticated session may
    # read it. Unauthenticated -> 401, never the data.
    if not authed():
