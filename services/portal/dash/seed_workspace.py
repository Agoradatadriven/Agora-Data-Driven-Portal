...
# Brand assets live in the repo-root assets/ folder (the brand kit). The seed runs from the REPO, so
# it READS those files and INLINES them into workspace/<c>.json (brand.agora_logo / brand.client_logo).
# The deployed container only bundles dash/, so it cannot read assets/ at runtime -- logos must be
...
#   * AGORA master logo : assets/logo.svg          -> brand.AGORA_LOGO_LIGHT
#   * Per-client logo   : assets/clients/<c>.svg   -> brand.monogram(display_name)
_CREATIVES = os.path.normpath(os.path.join(os.path.dirname(__file__), "..", "..", "..", "assets"))
...
def agora_logo():
    """The shared AGORA logo from the brand kit (assets/logo.svg), else the bundled master mark."""
    return _read_svg(os.path.join(_CREATIVES, "logo.svg")) or brand.AGORA_LOGO_LIGHT


def client_logo(client, display_name):
    """The per-client logo (assets/clients/<c>.svg), else a generated initials monogram."""
...
def reapply_brand(client, display_name=None):
    """Refresh ONLY the brand logos on an existing workspace from assets/ (no other field touched).

    Run after dropping/updating assets/logo.svg or assets/clients/<c>.svg. It does not clobber
    ...
    print("[seed_workspace] rebranded workspace/%s.json from assets/." % client)
    return 0
