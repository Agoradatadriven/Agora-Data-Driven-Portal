"""Wrap a raster logo (PNG/JPG) into a self-contained assets/clients/<key>.svg.

The Atrium seed reads ONLY assets/clients/<key>.svg and inlines it into the client's workspace
...
data: URI inside a tiny SVG wrapper -- self-contained, no external refs, exactly what the seed reads.
    ...
    python logo_to_svg.py <key> <path-to-image>      # -> assets/clients/<key>.svg
...
_CREATIVES_CLIENTS = os.path.normpath(
    os.path.join(os.path.dirname(__file__), "..", "..", "..", "assets", "clients")
)
...
def convert(key, image_path, max_edge=MAX_EDGE):
    """Write assets/clients/<key>.svg wrapping `image_path` (downscaled). Returns (path, (w, h))."""
