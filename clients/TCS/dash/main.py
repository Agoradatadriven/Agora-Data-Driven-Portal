"""Flask web service for the `tcs` client dashboard (Stage 3 of the data contract).

Serving model -- private bucket + Flask password gate, SSO additive:
  The per-client data JSON (`tcs.json`) lives in a PRIVATE GCS bucket
  (`agora-data-driven-tcs-dash`) and is NEVER public. This service renders a login,
...
# (tcs-dash-session-key) at deploy time. A missing secret is a hard misconfig.
SESSION_SECRET = os.environ.get("SESSION_SECRET", "")
# The dashboard's own password (mounted from tcs-dash-password). compare_digest below
...
GCS_BUCKET = os.environ.get("GCS_BUCKET", "agora-data-driven-tcs-dash")
DATA_OBJECT = os.environ.get("DATA_OBJECT", "tcs.json")

app = Flask(__name__)
app.secret_key = SESSION_SECRET

# Cookie hardening. SameSite=None + Secure is REQUIRED for the cross-subdomain portal
# flow (portal.agoradatadriven.com -> tcs.agoradatadriven.com): a Lax/Strict cookie
...
def _inline_logo(html):
    """Inline logo.avif as a data URI so the dashboard stays self-contained (no external
    asset fetch, matching the 'one self-contained dashboard.html' contract). Degrades to a
    1x1 transparent pixel if the asset is missing, so the <img> is never broken."""
    import base64
    try:
        with open(os.path.join(_HERE, "logo.avif"), "rb") as _lf:
            uri = "data:image/avif;base64," + base64.b64encode(_lf.read()).decode("ascii")
    except Exception:
        uri = "data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
    return html.replace("__TCS_LOGO_SRC__", uri)


DASHBOARD_HTML = _inline_logo(DASHBOARD_HTML)
...
def authed():
    """True iff this request is authenticated -- by its OWN session OR an additive portal SSO cookie.

    Wrapped in try/except returning False so SSO can NEVER raise into (and thus break) the
    password gate: if platform_sso ever throws, we fail CLOSED to the password path.
    """
    try:
        if session.get("ok") is True:
            return True
        return platform_sso.sso_allows(request)
    except Exception:
        return False
