# =============================================================================
# deploy_job_template.ps1 -- build + deploy the `template` client EXPORT job
...
#   the runtime SA (template-dash-job@), so a cloudbuild-driven DEPLOY would fail
...
$JOB     = "template-export"   # Cloud Run job name (<c>-export)
$SA      = "template-dash-job@agora-data-driven.iam.gserviceaccount.com"
...
Write-Host ""
Write-Host "[OK] template export job deploy complete (tag $SHA)" -ForegroundColor Green
