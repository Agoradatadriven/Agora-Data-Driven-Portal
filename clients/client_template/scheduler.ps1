...
#                  for the template client export job.
#
# Creates (or updates, idempotently) the Cloud Scheduler HTTP job
# `template-export-daily`, which POSTs the Run Admin API :run URI of the
# `template-export` Cloud Run job every 10 minutes. The export job is
...
# (deploy_template.ps1 grants that). We resolve it from the project number rather
