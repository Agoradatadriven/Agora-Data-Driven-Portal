# deploy_views_template.ps1 — re-apply this client's BigQuery views, then force
...
#   2. Re-run the export job `template-export` with FORCE_REBUILD=1.
#
...
# bucket keeps serving the STALE `template.json` built before the view changed.
...
# sql/  ->  client_template/  is the parent of $PSScriptRoot.
$ClientDir   = Split-Path -Parent $PSScriptRoot
$CreateViews = Join-Path $ClientDir "create_views.py"
# Repo root is clients/client_template/ -> clients/ -> <repo root>.
